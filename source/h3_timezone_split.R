...
# h3_timezone_split.R
#
# Hypothesis 3 - "Unexpectedness mediates" - split by timezone.
#
#   Oddity raises perceived unexpectedness, and unexpectedness carries the
#   effect of oddity on comfort.
#
# This is h3_full_set.R run once per level of `c_tmzone`:
#
#   "AU"   Australia
#   "UK"   United Kingdom
#   "US"   United States
...
# between the columns is descriptive only.
#
# THREE GROUPS, NOT TWO
#
#   With three levels there is no single contrast to inspect - there are three
#   pairwise ones, plus an omnibus question. This script tests none of them.
#   A formal test needs PROCESS Model 59 with a MULTICATEGORICAL moderator
#   (/mcw=1, which builds k-1 dummies and tests them jointly), the analogue of
#   h3_gender_split_model_59.R generalised past two groups. Ask for it if the
#   descriptive picture below turns out to be interesting.
#
#   Note also the group sizes are badly unbalanced - AU is roughly an eighth
#   the size of UK. Small groups produce wide bootstrap intervals, so a
#   "weaker" effect in AU may only mean a noisier estimate. Compare the
#   interval widths before comparing the point estimates.
#
...
#
# Usage:
#   Rscript source/h3_timezone_split.R      # run from the project root
#   source("source/h3_timezone_split.R")    # or from within RStudio
#
...
# re-set at the start of each group's bootstrap, so the groups do not share a
# random stream and any one of them can be reproduced in isolation.
#
...
# `c_tmzone` is a free-text column, so values are matched after trimming.
SPLIT_VAR <- "c_tmzone"
GROUPS <- list(
  list(value = "AU", label = "AU"),
  list(value = "UK", label = "UK"),
  list(value = "US", label = "US")
)

# Groups smaller than this get a warning in the report - not an error, but a
# reminder that the bootstrap interval is doing a lot of work.
SMALL_N <- 400
...
if (!all(seen %in% want)) {
  warning(SPLIT_VAR, " contains unhandled value(s): ",
          ...
          " - those rows are excluded from all groups.")
}
...
# HC4 in particular is the right choice for the small AU group: it is the
# variant designed to stay honest when a few high-leverage points would
# otherwise dominate the sandwich.
...
# wrapped in a function so it can be applied to each timezone group.
...
run_h3 <- function(dat, label) {
  ...
  # be reproduced without running the other groups first.
...
report_group <- function(r, split_value) {
  ...
  if (r$n < SMALL_N) {
    cat(sprintf("  NOTE: n = %d is small (< %d). Intervals below are wide;\n",
                r$n, SMALL_N))
    cat("        read them as imprecision, not as absence of an effect.\n")
  }
  if (r$n_fail > 0L) {
...
cat(sprintf("Model     Y = %s   X = %s   M = %s   covariate = %s\n",
            Y_VAR, X_VAR, M_VAR, COV_VAR))
cat(sprintf("Split     %s: %s   (%d groups)\n", SPLIT_VAR,
            paste(sprintf("\"%s\"", labels), collapse = ",  "), n_groups))
cat(sprintf("Inference %s standard errors; %d bootstrap samples; %d%% CI; seed %d per group\n",
            HC_TYPE, N_BOOT, CONF, SEED))
cat("\n")

cat("Group sizes\n")
group_n <- vapply(GROUPS,
                  function(g) sum(dat_all[[SPLIT_VAR]] == g$value),
                  numeric(1))
for (k in seq_len(n_groups)) {
  cat(sprintf("  %-14s %s = \"%s\"   n = %5d   (%s%% of %d)%s\n",
              labels[k], SPLIT_VAR, GROUPS[[k]]$value, group_n[k],
              num(100 * group_n[k] / n_complete, 1), n_complete,
              if (group_n[k] < SMALL_N) "  <- small" else ""))
}
if (any(group_n < SMALL_N)) {
  cat("\n  Group sizes are unbalanced. Bootstrap intervals scale with 1/sqrt(n),\n")
  cat("  so the smaller groups will show wider intervals for the same effect.\n")
}
cat("\n")
...
#
# Descriptive only. The groups were fitted independently, so a difference
# between the columns is NOT a test of moderated mediation - overlapping CIs
# and non-overlapping CIs are both uninformative about that question. With
# three groups there is not even a single contrast to look at.
#
# Laid out generically so it widens automatically if GROUPS gains a level.

COL_W    <- 25
CMP_RULE <- 18 + n_groups * (COL_W + 1)

pad <- function(s, w = COL_W) formatC(s, width = w)

cmp_line <- function(name, cells) {
  cat(formatC(name, width = -18))
  for (cell in cells) cat(" ", pad(cell), sep = "")
  cat("\n")
}

cmp_row <- function(name, getter) {
  cmp_line(name, vapply(labels, function(l) getter(results[[l]]), character(1)))
}

eff_ci <- function(est, lo, hi) {
  sprintf("%s %s", num(est, 3), ci(lo, hi, 3))
}
...
cmp_line("", labels)
cmp_line("Path", vapply(labels,
                        function(l) sprintf("n = %d", results[[l]]$n),
                        character(1)))
rule(CMP_RULE)
...
rule(CMP_RULE)
cmp_row("ab CI width", function(r) num(r$ab_ci[2] - r$ab_ci[1], 4))
...
cat("descriptive; this script does not test whether ", SPLIT_VAR,
    " moderates any path.\n", sep = "")
cat("Check the 'ab CI width' row before reading anything into a group whose\n")
cat("point estimate looks out of line - a wide interval means a noisy estimate.\n")
cat("For a real test use PROCESS Model 59 with a multicategorical moderator.\n")
cat("\n")
