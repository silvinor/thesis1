#!/usr/bin/env Rscript
# ---------------------------------------------------------------------------
# h3_Propensity_split.R
#
# Hypothesis 3 - "Unexpectedness mediates" - split by travel propensity.
#
#   Oddity raises perceived unexpectedness, and unexpectedness carries the
#   effect of oddity on comfort.
#
# This is h3_full_set.R run twice, once over each level of `c_propen`:
#
#   c_propen = 0  ->  "Adventurous"
#   c_propen = 1  ->  "Cautious"
#
...
# test of whether propensity moderates a path, that is PROCESS Model 59 (or a
...
#
# Usage:
#   Rscript source/h3_Propensity_split.R    # run from the project root
#   source("source/h3_Propensity_split.R")  # or from within RStudio
#
...
SPLIT_VAR <- "c_propen"
GROUPS <- list(
  list(value = 0, label = "Adventurous"),
  list(value = 1, label = "Cautious")
)
...
want <- vapply(GROUPS, function(g) g$value, numeric(1))
if (!all(want %in% seen)) {
...
# wrapped in a function so it can be applied to each propensity group.
...
cat(sprintf("Split     %s: %s\n", SPLIT_VAR,
            paste(vapply(GROUPS,
                         function(g) sprintf("%g = %s", g$value, g$label),
                         character(1)),
                  collapse = ",  ")))
